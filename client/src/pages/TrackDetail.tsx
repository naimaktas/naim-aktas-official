 ...
 * Track detail page with YouTube embed, track info, and navigation
 */
import { useEffect, useState } from "react";
import { Link, useParams } from "wouter";
import { tracks, socialLinks, getTrackBySlug, getYoutubeThumbnail, type Track } from "@/lib/tracks";
import { ArrowLeft, Youtube, Instagram, Facebook, Music, Play, ExternalLink, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function TrackDetail() {
  ...
  // Related tracks (same era, different tracks)
  const relatedTracks = tracks
    .filter(t => t.id !== track.id && t.youtubeId)
    .slice(0, 6);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              ...
              <div className="mb-8">
                ...
                <h1
                  className="text-3xl md:text-5xl font-bold text-[oklch(0.95_0.005_65)] leading-tight mb-4"
              ...
              {track.youtubeId ? (
                ...
                <div className="relative rounded-2xl overflow-hidden bg-[oklch(0.12_0.012_265)] border border-[oklch(1_0_0/8%)] mb-8 p-8">
                ...
                {prevTrack && (
                  <Link href={`/track/${prevTrack.slug}`} className="flex-1">
                    <div className="group flex items-center gap-3 p-4 rounded-xl border border-[oklch(1_0_0/8%)] bg-[oklch(0.12_0.012_265)] hover:border-[oklch(0.75_0.18_45/30%)] hover:bg-[oklch(0.15_0.012_265)] transition-all duration-200 cursor-pointer">
                      <ChevronLeft className="w-5 h-5 text-[oklch(0.45_0.01_265)] group-hover:text-[oklch(0.75_0.18_45)] transition-colors flex-shrink-0" />
                ...
                {nextTrack && (
                  <Link href={`/track/${nextTrack.slug}`} className="flex-1">
                    <div className="group flex items-center justify-end gap-3 p-4 rounded-xl border border-[oklch(1_0_0/8%)] bg-[oklch(0.12_0.012_265)] hover:border-[oklch(0.75_0.18_45/30%)] hover:bg-[oklch(0.15_0.012_265)] transition-all duration-200 cursor-pointer">
                      ...
                      <ChevronRight className="w-5 h-5 text-[oklch(0.45_0.01_265)] group-hover:text-[oklch(0.75_0.18_45)] transition-colors flex-shrink-0" />
                    </div>
                  </Link>
                )}
              </div>
                      ...
                      <div className="group flex items-center gap-3 p-3 rounded-xl border border-[oklch(1_0_0/6%)] bg-[oklch(0.12_0.012_265)] hover:border-[oklch(0.75_0.18_45/30%)] hover:bg-[oklch(0.15_0.012_265)] transition-all duration-200 cursor-pointer">
                        {related.youtubeId && (
                          <div className="relative w-16 h-10 rounded-lg overflow-hidden flex-shrink-0">
                            <img
                              src={getYoutubeThumbnail(related.youtubeId)}
                              alt={related.title}
                              className="w-full h-full object-cover"
                              onError={(e) => {
                                const target = e.target as HTMLImageElement;
                                if (target.src.includes('maxresdefault')) {
                                  target.src = `https://img.youtube.com/vi/${related.youtubeId}/hqdefault.jpg`;
                                }
                              }}
                            />
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-xs text-[oklch(0.75_0.005_65)] group-hover:text-[oklch(0.75_0.18_45)] transition-colors line-clamp-2 leading-snug" style={{ fontFamily: "'Cinzel', serif" }}>
                            {related.title}
                          </p>
                        </div>
                      </div>
