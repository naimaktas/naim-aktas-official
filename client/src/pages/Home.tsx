...
function HeroPlayer({ onTrackChange }: { onTrackChange?: (track: Track) => void }) {
const videoTracks = tracks.filter((t) => t.youtubeId).slice(0, 8);
const [current, setCurrent] = useState(0);
const [playing, setPlaying] = useState(false);

useEffect(() => {
if (videoTracks.length > 0 && onTrackChange) {
const nextIndex = (current + 1) % videoTracks.length;
onTrackChange(videoTracks[nextIndex]);
}
...
<h3 className="text-sm font-medium text-[oklch(0.92_0.005_65)] group-hover:text-[oklch(0.75_0.18_45)] transition-colors duration-200 line-clamp-2 leading-snug" style={{ fontFamily: "'Cinzel', serif" }}>{track.title}
        ...
        <div className="lg:col-span-3 flex items-center h-full reveal" style={{ animationDelay: "250ms" }}>
          <div className="p-4 rounded-xl border border-[oklch(1_0_0/6%)] bg-[oklch(0.12_0.012_265/40%)] w-full backdrop-blur-md text-left">
            ...
            <h4 className="text-xs font-semibold text-[oklch(0.85_0.005_65)] line-clamp-2" style={{ fontFamily: "'Cinzel', serif" }}>
              {nextTrack ? nextTrack.title : "Yükleniyor..."}
            </h4>
            <p className="text-[10px] text-[oklch(0.35_0.01_265)] mt-1.5">{nextTrack ? nextTrack.date : ""}</p>
          </div>
        </div>
